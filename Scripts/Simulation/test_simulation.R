...
unclbr[, `:=` (af_prvl_correction = af_incd * (af_incd_clbr_fctr - 1),
              af_incd = NULL, intercept_unclbr = NULL, trend_unclbr = NULL)]
...
benchmark[, mu_modelled_bm := exp(intercept_bnchmrk + (trend_bnchmrk * log(year)))]

years <- 2013:2050

# sex_test <- "men"
sex_test <- "women"
test <- data.table(year = years)
test <- merge(test, benchmark[sex==sex_test, .(year, mu)], by="year") 
test <- merge(test, benchmark[sex==sex_test, .(year, mu_modelled_bm)], by="year") 
test

plot(test[, year], test[, mu])
lines(test[, year], test[, mu_modelled_bm])
