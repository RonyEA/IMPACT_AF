    ...
    public = list(
      ...
      initialize = function(sRelativeRiskByPopulationSubsetForExposureFilePath, design) {
        ...
        if (!file.exists(sRelativeRiskByPopulationSubsetForExposureFilePath)) {
          ...
          if ("smk" %in% names(dtRelativeRiskByPopulationSubset))
            dtRelativeRiskByPopulationSubset[, smk :=
                               factor(smk, levels = 1:3)]
        ...
        if ("smk" %in% names(private$input_rr))
          private$input_rr[, smk :=
                         factor(smk, levels = 1:3)]
