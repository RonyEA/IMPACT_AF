...
cat("Initialising IMPACTaf model...\n\n")
...
dependencies(yaml::read_yaml("./dependencies.yaml"))
  ...
  snfile <- "./Rpackage/.IMPACTaf_model_pkg_snapshot.qs"
  if (file.exists(snfile)) snapshot <- changedFiles(qread(snfile))


  if (!nzchar(system.file(package = "IMPACTaf")) ||
    ...
    if (nzchar(system.file(package = "roxygen2"))) {
      file.remove(list.files(
        path = "./Rpackage/IMPACTaf_model_pkg/src",
        pattern = ".o$|.so$|.dll$", full.names = TRUE
      ))
    }
    roxygen2::roxygenise("./Rpackage/IMPACTaf_model_pkg/", clean = TRUE)
    ...
    detach_package(IMPACTaf)
    remotes::install_local("./Rpackage/IMPACTaf_model_pkg/",
      force = TRUE,
      upgrade = "never"
    )
